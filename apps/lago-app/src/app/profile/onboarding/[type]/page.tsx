...
> = {
  personal_seller: {
    title: "个人卖家入驻申请",
    description: "提交身份证明后即可开通个人卖家权限，便于发布和管理闲置商品。",
  ...
  personal_service_provider: {
    title: "个人服务商入驻申请",
    description: "适用于维修、安装、保洁等个人服务者，需实名和选择服务类别。",
...
function OnboardingFormContent() {
  ...
  const parsedType = params.type as OnboardingApplicationType;
  ...
  const [businessLicense, setBusinessLicense] = useState<UploadedMedia | null>(
    null
  );
  ...
  const canSubmit = useMemo(() => {
    ...
    if (
      requiresIdentity &&
      (!form.fullName || !form.idNumber || !idCardFront || !idCardBack)
    ) {
      return false;
    }
    if (
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (extraAttachments.length > 0) {
      documents.attachments = extraAttachments.map((item) =>
        serializeMedia(item)
      );
    }
          ...
          {requiresBusiness && (
            <>
              <FormField
                ...
                onChange={(val) =>
                  setForm((prev) => ({ ...prev, businessName: val }))
                }
              />
              <FormField
                label="营业执照注册号"
                ...
                onChange={(e) =>
                  setForm((prev) => ({
                    ...
                    serviceCategory: e.target
                      .value as OnboardingApplicationCreateRequestServiceCategory,
                  }))
                }
